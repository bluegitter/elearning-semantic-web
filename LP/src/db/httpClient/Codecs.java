...
public class Codecs
{
	...
	try
	    { return new String(base64Encode(str.getBytes("UTF-8")), "UTF-8"); }
	...
	for (idx=0; !done; idx++)
	{
		...
		hdr      = new String(data, start, next-2-start, "UTF-8");
		start    = next;
	    ...
	    {
		value = new String(data, start, end-start, "UTF-8");
	    }
	...
	byte[] boundary  = Boundary.getBytes("UTF-8"),
	       cont_disp = ContDisp.getBytes("UTF-8"),
	       cont_type = ContType.getBytes("UTF-8"),
	...
	for (int idx=0; idx<files.length; idx++)
	{
		...
		len += fname.getBytes("UTF-8").length + file.length();
	...
	NewBound: for (int new_c=0x30303030; new_c!=0x7A7A7A7A; new_c++)
	{
		...
		System.arraycopy(opts[idx].getName().getBytes("UTF-8"), 0, res, pos, nlen);
		pos += nlen;
		...
		System.arraycopy(files[idx].getName().getBytes("UTF-8"), 0, res, pos, nlen);
		pos += nlen;
		...
		nlen = fnameBytes.length;
		System.arraycopy(fnameBytes, 0, res, pos, nlen);
		pos += nlen;
		...
		if (ct != null)
		{
		    ...
		    System.arraycopy(ct.getBytes("UTF-8"), 0, res, pos, ct.length());
		    pos += ct.length();
		}
	...
	ct_hdr[0] = new NVPair("Content-Type",
			       ...
			       new String(boundary, 4, boundary.length-4, "UTF-8"));
	...
	if (len > 0)
	{
	    ...
	    try
		{ System.arraycopy(hex_len.getBytes("UTF-8"), 0, res, r_off, hlen); }
	    catch (UnsupportedEncodingException uee)
		{ throw new Error(uee.toString()); }
	    r_off += hlen;
	    res[r_off++] = (byte) '\r';
	...
	if (last)
	{
		...
		try
		    { System.arraycopy(ftrs[idx].getName().getBytes("UTF-8"),
				       0, res, r_off, nlen); }
		catch (UnsupportedEncodingException uee)
		    { throw new Error(uee.toString()); }
		r_off += nlen;
		...
		try
		    { System.arraycopy(ftrs[idx].getValue().getBytes("UTF-8"),
				       0, res, r_off, vlen); }
		catch (UnsupportedEncodingException uee)
		    { throw new Error(uee.toString()); }
		r_off += vlen;
	...
	if (ch != '\n'  &&  (ch != '\r'  ||  input.read() != '\n'))
	    throw new ParseException("Didn't find valid chunk length: " +
				     new String(hex_len, 0, off, "UTF-8"));

	// parse chunk length

	try
	    { return Long.parseLong(new String(hex_len, 0, off, "UTF-8").trim(),
				    16); }
